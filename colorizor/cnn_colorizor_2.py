...
class Net():
    ...
    def batch_norm(self, scope, x):
        return tf.contrib.layers.batch_norm(x, center=True, scale=True, updates_collections=None, is_training=False, trainable=False, scope=scope)
...
def colorize(img, ckpt, pts):
    ...
    output_ab = output.resize(output, (h, w))
